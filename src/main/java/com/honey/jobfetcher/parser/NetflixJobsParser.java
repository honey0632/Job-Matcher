...
public class NetflixJobsParser {
    public List<Jobs> parse(String html) {
        ...
        for (Element jobElement : jobElements) {
            ...
            String url = "https://jobs.netflix.com" + jobElement.select("a.job-link").attr("href");

            job.setTitle(title);
            job.setLocation(location);
            job.setJobUrl(url);
            job.setExternalId(JobSource.NETFLIX.name() + ":" + id);
            job.setSource(JobSource.NETFLIX.name());
