...
public class SwiggyJobsParser {
    public List<Jobs> parse(String html) {
        ...
        for (Element jobElement : jobElements) {
            ...
            String url = "https://careers.swiggy.com" + jobElement.select("a.job-link").attr("href");

            job.setTitle(title);
            job.setCompany("Swiggy");
            job.setLocation(location);
            job.setJobUrl(url);
            job.setExternalId(JobSource.SWIGGY.name() + ":" + id);
            job.setSource(JobSource.SWIGGY.name());
