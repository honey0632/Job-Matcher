...
@Component
public class RazorpayJobsParser {
    public List<Jobs> parse(String html) {
        ...
        for (Element jobElement : jobElements) {
            ...
            String url = "https://razorpay.com" + jobElement.select("a.job-link").attr("href");

            job.setTitle(title);
            job.setCompany("Razorpay");
            job.setLocation(location);
            job.setJobUrl(url);
            job.setExternalId(JobSource.RAZORPAY.name() + ":" + id);
            job.setSource(JobSource.RAZORPAY.name());
