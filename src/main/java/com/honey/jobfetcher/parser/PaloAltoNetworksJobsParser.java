...
@Component
public class PaloAltoNetworksJobsParser {
    public List<Jobs> parse(String html) {
        ...
        for (Element jobElement : jobElements) {
            ...
            String url = "https://www.paloaltonetworks.com" + jobElement.select("a.job-link").attr("href");

            job.setTitle(title);
            job.setLocation(location);
            job.setJobUrl(url);
            job.setExternalId(JobSource.PALO_ALTO_NETWORKS.name() + ":" + id);
            job.setSource(JobSource.PALO_ALTO_NETWORKS.name());
