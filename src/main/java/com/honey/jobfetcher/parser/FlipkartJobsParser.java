...
@Component
public class FlipkartJobsParser {
    public List<Jobs> parse(String html) {
        ...
        for (Element jobElement : jobElements) {
            ...
            String url = "https://www.flipkartcareers.com" + jobElement.select("a.job-link").attr("href");

            job.setTitle(title);
            job.setCompany("Flipkart");
            job.setLocation(location);
            job.setJobUrl(url);
            job.setExternalId(JobSource.FLIPKART.name() + ":" + id);
            job.setSource(JobSource.FLIPKART.name());
