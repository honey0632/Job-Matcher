...
@Component
public class MicrosoftJobsParser {
    public List<Jobs> parse(String html) {
        ...
        for (Element jobElement : jobElements) {
            ...
            String url = "https://careers.microsoft.com" + jobElement.select("a.job-link").attr("href");

            job.setTitle(title);
            job.setLocation(location);
            job.setJobUrl(url);
            job.setExternalId(JobSource.MICROSOFT.name() + ":" + id);
            job.setSource(JobSource.MICROSOFT.name());
