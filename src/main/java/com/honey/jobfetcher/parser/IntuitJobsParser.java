...
public class IntuitJobsParser {
    public List<Jobs> parse(String html) {
        ...
        for (Element jobElement : jobElements) {
            ...
            String url = "https://www.intuit.com" + jobElement.select("a.job-link").attr("href");

            job.setTitle(title);
            job.setLocation(location);
            job.setJobUrl(url);
            job.setExternalId(JobSource.INTUIT.name() + ":" + id);
            job.setSource(JobSource.INTUIT.name());
