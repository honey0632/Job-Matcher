...
@Component
public class PostmanJobsParser {
    public List<Jobs> parse(String html) {
        ...
        for (Element jobElement : jobElements) {
            ...
            String url = "https://www.postman.com" + jobElement.select("a.job-link").attr("href");

            job.setTitle(title);
            job.setCompany("Postman");
            job.setLocation(location);
            job.setJobUrl(url);
            job.setExternalId(JobSource.POSTMAN.name() + ":" + id);
            job.setSource(JobSource.POSTMAN.name());
