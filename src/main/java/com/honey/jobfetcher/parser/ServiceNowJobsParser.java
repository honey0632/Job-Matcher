...
@Component
public class ServiceNowJobsParser {
    public List<Jobs> parse(String html) {
        ...
        for (Element jobElement : doc.select("div.job-card")) {
            ...
            job.setJobUrl("https://www.servicenow.com" + jobElement.select("a.job-link").attr("href"));
            job.setExternalId(JobSource.SERVICENOW.name() + ":" + id);
            job.setSource(JobSource.SERVICENOW.name());
            jobs.add(job);
        }
